...
public abstract class TemplateAdapterOperations<E, K, V> {
    ...
    protected TemplateAdapterOperations(DynamoDbEnhancedAsyncClient ddb, DynamoDbAsyncClient ddp, ObjectMapper mapper, Function<V, E> toEntityFn, String tableName, String... index) {
        this.toEntityFn = toEntityFn;
        this.mapper = mapper;
        ParameterizedType genericSuperclass = (ParameterizedType) this.getClass().getGenericSuperclass();
        this.dataClass = (Class<V>) genericSuperclass.getActualTypeArguments()[2];
        customerTable = ddb.table(tableName, TableSchema.fromBean(dataClass));
